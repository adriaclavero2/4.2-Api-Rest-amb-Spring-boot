...
> A production-ready RESTful API built with Spring Boot and MySQL for managing a fruit shop's inventory and its supplier network.
...
Built following clean architecture principles with a layered design, the DTO pattern, relational data modeling via JPA, and a complete TDD testing suite.
...
Preconfigured in `docker-compose.yml`. Override as needed:
...
No local Java or MySQL required. Builds the image and provisions the database automatically.
...
Covers both the Controller and Service layers following a TDD approach.
